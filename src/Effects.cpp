...
void Effects::burst(sf::Vector2f pos,sf::Color c,int count,float speed){
    ...
    for(int i=0;i<count&&particles_.size()<1400;++i){ float ang=a(rng()), sp=speed*s(rng()); const float life=l(rng());particles_.push_back({pos,{std::cos(ang)*sp,std::sin(ang)*sp},c,life,life,z(rng()),a(rng())*180.f/PI_F,(s(rng())-.65f)*240.f,.08f,0.f}); }
}
void Effects::trail(sf::Vector2f pos,sf::Vector2f velocity,sf::Color color,float size,float life,float gravity){if(particles_.size()<1400)particles_.push_back({pos,velocity,color,life,life,size,0.f,45.f,.25f,gravity});}
...
void Effects::clear(){ particles_.clear(); rings_.clear(); tracers_.clear(); texts_.clear(); shakeRequest_=0.f; }
void Effects::update(float dt){
    for(auto& p:particles_){ p.life-=dt; p.vel.y+=p.gravity*dt;p.pos+=p.vel*dt;p.rotation+=p.angularVelocity*dt; p.vel*=std::pow(p.drag,dt); }
...
void Effects::draw(sf::RenderTarget& rt,const aegis::ui::TextService* text) const{
    ...
    for(const auto& p:particles_){ sf::RectangleShape s({p.size*1.8f,p.size*.72f}); s.setOrigin(p.size*.9f,p.size*.36f); s.setPosition(p.pos);s.setRotation(p.rotation); auto c=p.color; c.a=static_cast<sf::Uint8>(255*clampf(p.life/p.maxLife,0,1)); s.setFillColor(c); rt.draw(s); }
