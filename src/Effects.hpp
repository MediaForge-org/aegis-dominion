...
struct Particle { sf::Vector2f pos, vel; sf::Color color; float life=1.f,maxLife=1.f,size=4.f,rotation=0.f,angularVelocity=0.f,drag=.08f,gravity=0.f; };
...
public:
    ...
    void trail(sf::Vector2f pos, sf::Vector2f velocity, sf::Color color, float size = 4.f, float life = .35f, float gravity = 0.f);
    ...
    void clear();
    void shake(float strength) { shakeRequest_ = std::max(shakeRequest_, strength); }
    float consumeShake() { const float value = shakeRequest_; shakeRequest_ = 0.f; return value; }
private:
    std::vector<Particle> particles_;
    std::vector<RingFx> rings_;
    std::vector<TracerFx> tracers_;
    std::vector<FloatTextFx> texts_;
    float shakeRequest_ = 0.f;
};
